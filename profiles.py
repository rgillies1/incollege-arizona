...
def editProfile():#Edit logged in user profilepro = getProfile()
  ...
  def prof():  
    ...
    while selection not in selections:
      selection = input("Choose option from above or Type 'Exit' to exit: ")
    if(selection=='Exit'):
      return None
    if(selection=='1'):
  ...
  def jobExp():
    ...
    while selection not in selections:
      ...
      if(selection.isnumeric()):
        if(selection=='0'and len(jobs)==3 or (int(selection)>len(jobs))):#Checks selection is valid
          selection=-1
    if(selection=='Exit'):
      return None
    if(selection=='0'):
      ...
      while opt !="Exit":
        ...
        while opt not in validOpts:
          opt = input("Choose option from above or Type 'Exit' to exit: ")
        if(opt=='1'):
...
def friendProfiles():#Prints friends and allows to choose which friend profile to view
  friends = getFriends()
  print("")
  if (len(friends) == 0):
...
def profile(userId=-1):#Displays profile of selected userId.  userId=-1 means logged in user
  ...
  if(jobs!=[] and jobs!=[(None,None,None,None,None,None,None)]):#Job Experience check
    print("**Job Experience**")
    for i in range(0,len(jobs)):
      jobNum,job,boss,start,end,loc,desc=jobs[i]
      if(job!=None):
  ...
  if(userId==-1):
    ...
    while choice not in validChoices:
      choice = input("Please choose from the options above. Type 'Exit' to exit: ")
    if(choice=='1'):
      friendProfiles()
    elif(choice=='2'):
      friendNetwork()
