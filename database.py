...
def tables(connection=sqlite3.connect("inCollege.db")):
  ...
  timeOfLastApplication INTEGER DEFAULT 0 NOT NULL,
  --major TEXT, 
  --college TEXT, 
  FOREIGN KEY(id) REFERENCES UserLogin(id));""")
  ...
  # 1 - New message
  # 2 - New job post
  # 3 - Applied job was deleted
  # 4 - No job application in the last 7 days
...
def newAccount(username,
  ...
  notifyUsers(
    "{0} {1} ({2}) has joined InCollege!".format(firstName, lastName,
                                                 username), -1)
...
def newJobPost(jobTitle,
  ...
  connection.execute(
    ...
    .format(jobTitle, jobDescription, jobEmployer, jobLocation, jobSalary,
            fullName, userId))
  connection.commit()
  notifyUsers("A new job has been posted!", 2)
  ...
  if (requestCheck1 == None and friendCheck and requestCheck2 == None
    ...
    connection.execute(
      "INSERT INTO FriendRequest (sendId,recieveId) VALUES ({0},{1});".format(
        userId, friendId))
  connection.commit()
  # If we wanted to list how many friend requests they have, it would have to happen here
  sendNotification("You have one or more pending friend requests!", 0,
                   friendId)
...
def removeFriendRequest(sendId,
  ...
  # "receive" is spelled wrong in the db ¯\_(ツ)_/¯
  friendRequests = connection.execute(
    "SELECT * FROM FriendRequest WHERE recieveId = '{0}';".format(
      recieveId)).fetchall()
  if len(friendRequests) == 0:
    connection.execute(
      "DELETE FROM Notifications WHERE userId = '{0}' AND notificationType = '{1}';"
      .format(recieveId, 0))
    connection.commit()
...
def getUserId(username="%", connection=sqlite3.connect("inCollege.db")):
  if (username == -1):
    username = authLogin()()[1]
  if (username == None):
    pass
  return connection.execute(
    "SELECT id FROM UserLogin WHERE username = '{0}';".format(
...
def apply(jobPostId,
  ...
  connection.execute(
    "UPDATE UserData SET timeOfLastApplication = '{0}' WHERE id = '{1}';".
    format(time.time(), userId))
  connection.commit()
...
def removeJobPost(
  ...
  positionName = connection.execute(
    "SELECT position FROM JobBoard WHERE jobPostId = '{0}';".format(
      jobPostId)).fetchone()[0]
  connection.execute(
  ...
  appliedIds = connection.execute(
    "SELECT id FROM Applications WHERE jobPostId = '{0}';".format(
      jobPostId)).fetchall()
  for id in appliedIds:
    sendNotification(
      "The following job post that you applied to was deleted: {0}".format(
        positionName), 3, id[0])
...
def sendMessage(receiverId,
  ...
  connection.commit()
  sendNotification("You have one or more unread message(s)!", 1, receiverId)
...
def markAsRead(messageId, connection=sqlite3.connect("inCollege.db")):
  ...
  unreadMessages = getInbox(unreadOnly=True)
  if len(unreadMessages) == 0:
    print("DEBUG")
    userId = connection.execute(
      "SELECT receiverId FROM Inbox WHERE messageId = '{0}';".format(
        messageId)).fetchone()[0]
...
def userRecords(
  userId=-1,
  connection=sqlite3.connect(
    "inCollege.db")  #Should it be usernames or name names?
):  #Returns all records in a list of tuples
  if (userId == -1):
    userId = authUser()[1]
  if (userId == None):
    pass
  return connection.execute(
    "SELECT id,username FROM UserLogin WHERE id != {0};".format(
      userId)).fetchall()
...
def sendNotification(notification,
  ...
  oldNotifId = connection.execute(
    "SELECT notificationId FROM Notifications WHERE userId = '{0}' AND notificationType = '{1}';"
    .format(userId, notifType)).fetchone()

  if (oldNotifId == None or notifType == -1):
    connection.execute(
      "INSERT INTO Notifications (userId, notificationType, notification) VALUES ('{0}', '{1}', '{2}');"
      .format(userId, notifType, notification))
    connection.commit()
  else:
    connection.execute(
      "UPDATE Notifications SET notification = '{0}' WHERE userId = '{1}' AND notificationType = '{2}';"
      .format(notification, userId, notifType))
    connection.commit()


# Get's a user's pending notifications
# By default, this will also delete the following notification categories so that they only appear once:
# 2 - New job post
# 3 - Applied job was deleted
# -1 - New user
def getNotifications(clearTempNotifications=True,
                     userId=-1,
                     connection=sqlite3.connect("inCollege.db")):
  if (userId == -1):
    userId = authUser()[1]
  notifs = connection.execute(
    "SELECT notification FROM Notifications WHERE userId = '{0}';".format(
      userId)).fetchall()
  if clearTempNotifications:
    connection.execute(
      "DELETE FROM Notifications WHERE userId = '{0}' AND (notificationType = '{1}' OR notificationType = '{2}' OR notificationType = '{3}' OR notificationType = '{4}');"
      .format(userId, 2, 3, -1, 6))
    connection.commit()
  if len(notifs) < 1:
    return []
  else:
    return list(zip(*notifs))[0]


# Send a notification to the list of userIds passed
# If no list is provided, notification is sent to all users
def notifyUsers(notification, notificationType, idsToNotify=[]):
  if idsToNotify == []:
    idsToNotify = map(
      operator.itemgetter(0), allRecords("UserData")
    )  # get all user records and only keep the first entry of each tuple

  for id in idsToNotify:
    sendNotification(notification, notificationType, id)


def getNumAppliedJobs(userId=-1, connection=sqlite3.connect("inCollege.db")):
  if (userId == -1):
    userId = authUser()[1]
...
def userHasAppliedThisWeek(userId=-1,
                           connection=sqlite3.connect("inCollege.db")):
  if (userId == -1):
    userId = authUser()[1]
  lastAppTime = connection.execute(
    "SELECT timeOfLastApplication FROM UserData WHERE id = '{0}';".format(
      userId)).fetchone()[0]

  currentTime = time.time()

  return currentTime - lastAppTime < 604800
